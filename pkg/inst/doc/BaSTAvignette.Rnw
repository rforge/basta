...
In this case, we are also including an interaction between sex and weight. For further details on how to specify a formula in R, type \mverb{help(formula)} in the R console. 
...
If argument \mverb{silent} is set as \mverb{FALSE}, then the function prints out a range of descriptive statistics about the dataset, exemplified as follows:
...
plot(basta.out)
@
This produces the following plot of traces for the mortality parameters:
